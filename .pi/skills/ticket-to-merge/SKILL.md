...
- Read the Linear issue, including acceptance criteria, milestone, and relevant comments.
- Confirm the issue belongs to **Website AKG**; the AKG team also contains unrelated projects.
...
- Reuse existing components and patterns; prefer semantic HTML and platform features, then local UI primitives.
- Add the smallest regression check for non-trivial behavior.
- Run the repository's `quality` script with the package manager declared in `package.json`. Before the application scaffold exists, run the smallest relevant checks plus `git diff --check`.
